...
Going live is six numbered scripts plus two short manual stops in browser dashboards. The scripts handle every part that's automatable.
...
| 3 | **Push code to GitHub** | Installs git+gh, creates public repo, pushes | `wizard/3-push-to-github.bat` |
| — | *Manual: create Cloudflare account* | Sign up at https://dash.cloudflare.com/sign-up (~2 min) | — |
| 4 | **Cloudflare bootstrap** | Adds bpleon.com to Cloudflare via API, copies nameservers to clipboard, opens Squarespace | `wizard/4-cloudflare-bootstrap.bat` |
| — | *Manual: paste nameservers into Squarespace* | Step-by-step shown by the script (~30 sec) | — |
| — | *Manual: connect Pages to GitHub repo in Cloudflare dashboard* | 5 clicks, ~2 min — see Phase 5b below | — |
| 5 | **Attach domain to Pages** | Polls DNS, waits for zone active, attaches bpleon.com + www to Pages project | `wizard/5-attach-domain.bat` |
| 6 | **Verify it's live** | Hits every URL, checks DNS, SSL, status codes | `wizard/6-verify-live.bat` |

*Personalize the placeholders* (`YOURNAME`, `[City]`, `[Firm]`, etc.) can happen anytime — locally before the first push, or directly in GitHub.com / github.dev once the repo is up. See "Editing later" below.
...
markets.html      Live markets dashboard (TradingView widgets)
toolkit.html      Templates + interactive tools index
dcf.html          Interactive DCF calculator (vanilla JS)
reports.html      Long-form research PDFs
projects.html     Full project list
writing.html      Writing index (mirrors Substack posts)
contact.html      Contact / links
now.html          What you're working on right now
disclaimer.html   "Not investment advice" disclaimer
404.html          Friendly not-found page
style.css         All styling — light + dark, responsive
script.js         Tiny JS — year, ticker tape with theme detect
...
assets/toolkit/   Drop .xlsx templates here
assets/reports/   Drop .pdf reports here
.gitignore        Stuff git should ignore
README.md         This file
```

**Adding content** is mostly copy-a-block-and-edit:
- New writing entry → add `<li>` to `writing.html` + `<item>` to `feed.xml`
- New project → add `<article>` to `projects.html`
- New template → drop `.xlsx` in `assets/toolkit/` + add `<li>` to `toolkit.html`
- New report → drop `.pdf` in `assets/reports/` + add `<li>` to `reports.html`
- Update /now → edit `now.html`
- Tune DCF defaults → edit the `value=` attrs at the top of `dcf.html`

After any edit: run `wizard/sync-to-github.bat` and the live site updates in ~20 seconds.

**Optional: enable Cloudflare Web Analytics** (free, privacy-friendly, ~30 sec setup). In your Cloudflare dashboard → Analytics & Logs → Web Analytics → Add a site → enter `bpleon.com`. Cloudflare gives you a tiny `<script>` snippet — paste it just before `</body>` in any pages you want tracked (or just `index.html` and `writing.html`). Privacy-first, no cookies, GDPR fine.

All internal links are relative, so you can preview by just **double-clicking `index.html`** — no server required.

---

## Phase 1 — Confirm you still own bpleon.com (1 min)
...
- **"Domain expires in less than 30 days. Renew it FIRST."** → log in at https://account.squarespace.com → Domains → bpleon.com → Renew. Then re-run the script.
- **"It looks like bpleon.com may NOT be registered anymore."** → first check Squarespace (you might still own it but DNS is just down). If the domain has truly been released, you can re-register it cheaply — Cloudflare Registrar (~$10/yr) is the lowest-margin option; Namecheap and Porkbun are also fine.
...
### 5c. Add bpleon.com to Cloudflare (5 min)

1. Cloudflare dashboard → **Websites** (left sidebar) → **+ Add a site**.
2. Enter `bpleon.com` → continue.
...
1. https://account.squarespace.com → **Domains** → click `bpleon.com`.
...
### 5e. Attach bpleon.com to your Pages project (2 min)

1. Cloudflare → **Workers & Pages** → click your `bpleone` project.
2. **Custom domains** tab → **Set up a custom domain**.
3. Enter `bpleon.com` → **Continue** → **Activate domain**.
4. Repeat for `www.bpleon.com` (the `_redirects` file in the repo will 301 www → root automatically).
...
It checks DNS resolution, HTTPS reachability, and that every page on bpleon.com returns 200. If anything fails it tells you which URL and most often the cause is "DNS still propagating" — wait 30 minutes and re-run.

When all checks pass: 🎉 you're done. Visit https://bpleon.com from any device.
...
DNS propagation. Wait 1–24 hours. Use https://dnschecker.org/#NS/bpleon.com to watch it propagate globally.
...
**I want a custom email like brandon@bpleon.com.**
Cloudflare Email Routing is free. In the Cloudflare dashboard → bpleon.com → **Email** → enable. Forward `brandon@bpleon.com` (and any other address) to your gmail. Receiving only — sending requires a paid email service like Fastmail or Google Workspace.
